...
class MachineStats(db.Model):
    ...
    machine = db.relationship(
        "Machines", backref=db.backref("stats", uselist=False)
    )
...
class MachineHistory(db.Model):
    ...
    def get_plots(stat):
        ...
        if stat == "oee":
            ...
            mapper = {
                name: run_rate * 60 for name, run_rate in ideal_run_rates
            }
            df["value"] = df.apply(
                lambda x: x["value"] / mapper.get(x["name"]), axis=1
            )
            df["value"] *= 100
...
class CurrentMachineStatus(db.Model):
    ...
    machine = db.relationship(
        "Machines", backref=db.backref("status", uselist=False)
    )
...
class Machines(db.Model):
    ...
    def update_status(self):
        ...
        if self.status.is_down:
            if (
                dt.datetime.utcnow() - self.status.last_down
            ).total_seconds() > self.stats.min_downtime_secs:
    ...
    def reset_status(self):
        ...
        old = MachineHistory.query.filter(
            and_(
                MachineHistory.datetime <= cutoff,
        ...
        self.history.append(
            MachineHistory(
                datetime=dt.datetime.utcnow().replace(
    ...
    def get_average_hourly_production(self, lookback_weeks=4):
        MH = MachineHistory
        history = (
            db.session.query(func.avg(MH.product_count))
            .filter(and_(MH.machine_id == self.id, MH.product_count > 0))
            .first()
        )
    ...
    def get_current_status():
        ...
        for machine in machines:
            ...
            if is_down:
                total_secs_down = int(
                    (
                        dt.datetime.utcnow() - machine.status.last_down
                    ).total_seconds()
                )
    ...
    def _backdate_production(days=30):
        ...
        start = dt.datetime.utcnow().replace(
        ...
        for machine in machines:
            ...
            hourly_production = np.full(
                len(daterange), (stats.ideal_run_rate * 60 * stats.efficiency)
            )
            ...
            scenario_choices = np.random.choice(
                len(scenarios), len(daterange), replace=True
            )
            down_counts = [
                scenarios[x]["down_count"] for x in scenario_choices
            ]
            down_secs = [scenarios[x]["down_secs"] for x in scenario_choices]

            # Scale hourly productivity to account for downtime
            scaling = np.array(
                [scenarios[x]["production_scaler"] for x in scenario_choices]
            )
    ...
    def _set_status():
        ...
        for machine in machines:
            history = (
                MachineHistory.query.filter_by(machine_id=machine.id)
...
class Problem:
    def __init__(self, request):
        ...
        self.product_name_map = dict(
            zip(self.products, current_app.config["PRODUCT_NAMES"])
        )
    ...
    def parse_request(self):
        ...
        for i, machine in enumerate(current_app.config["MACHINE_NAMES"]):
            ...
            try:
                ...
                self.machine_productivity[self.machines[i].id] = self.machines[
                    i
                ].get_average_hourly_production()
            except Exception:
        ...
        for i, machine in enumerate(current_app.config["MACHINE_NAMES"]):
            ...
            if shift_name and shift_name in valid_shift_names:
                self.machine_shifts[self.machines[i].id] = current_app.config[
                    "SHIFT_HOURS"
                ][shift_name]

        # grab forecast
        try:
            forecast = pd.read_html(self.req.get("finalised_forecast_table"))[
                0
            ]
        except Exception:
        ...
        forecast = (
            forecast.set_index("Product Name")
            .T.set_index(pd.DatetimeIndex(date_range))
            .cumsum()
        )
    ...
    def _build_productivity_array(
        self, ave_production, shift_overlay, num_weeks
    ):
    ...
    def _build_productivity_map(self):
        for machine in self.machines:
            ...
            expanded = self._build_productivity_array(
                self.machine_productivity[machine.id], shift_overlay, 5
            )
...
class Solver:
    def __init__(self, problem):
        ...
        self.knockout_hours = {
            product_id: 1 for product_id in self.problem.products
        }

        # Add a null product to all machine capabilities
        self.machine_products = {
            product_id: products + [0]
            for product_id, products in self.problem.machine_products.items()
        }
        ...
        self.lookahead_hours_penalty_dict = {
            product_id: 24 for product_id in self.problem.products
        }
        ...
        # Swaps
        self.machine_swaps = np.random.choice(
            self.machine_ids, self.iterations, replace=True
        ).tolist()
        ...
        self.wip_cost_contributions = np.full(
            len(self.problem.products) + 1, 0
        ).astype(np.float64)
        ...
        self.solution = {
            machine.id: np.full(len(self.problem.daterange), 0)
        ...
        self.wip_production = {
            product_id: np.full(len(self.problem.daterange), 0)
            for product_id in self.problem.products
        }
        self.wip_demands = {
            product_id: self.problem.forecast[product_id][1:]
            for product_id in self.problem.products
        }
    ...
    def _get_swap_indices(self):
        ...
        for machine_id, productivity in self.problem.productivity_map.items():
            ...
            for k, g in groupby(enumerate(arr), lambda x: x[0] - x[1]):
                ...
                chunked_swaps = [
                    chunk(item, self.min_swap_hours) for item in swaps
                ]
                start_indices = [
                    int(i[0]) for sublist in chunked_swaps for i in sublist
                ]
    ...
    def _build_initial_solution(self):
        for machine in self.problem.machines:
            ...
            for index in swap_indices:
                ...
                self.solution[machine.id][
                    index : index + self.min_swap_hours
                ] = product
    ...
    def _get_initial_productivity(self):
        ...
        for product_id in self.problem.products:
            ...
            if knockout_hours:
                ...
                production = np.where(
                    local_solution == product_id, local_productivity, 0
                ).astype(np.float64)
    ...
    def _get_initial_solution_cost(self):
        for product_id in self.problem.products:
            ...
            initial_cost = self._soft_solution_costs(product_id, production)
    ...
    def _add_remove_production(self, wip_modified):
        ...
        for bundle in wip_modified:
            ...
            if old_wip != 0:
                ...
                if old_wip != prior_product:
                    old_knockout = self.knockout_hours.get(
                        old_wip, self.default_knockout
                    )
                else:
                    old_knockout = 0
                ...
                wip_production = modified_wip_production.get(
                    old_wip, self.wip_production[old_wip]
                ).copy()
            ...
            if new_wip != 0:
                ...
                if new_wip != prior_product:
                    new_knockout = self.knockout_hours.get(
                        new_wip, self.default_knockout
                    )
                else:
                    new_knockout = 0
                ...
                # made some alteration on a previous iteration
                wip_production = modified_wip_production.get(
                    new_wip, self.wip_production[new_wip]
                ).copy()
                ...
                machine_soln = modified_solution.get(
                    machine, self.solution[machine]
                ).copy()
            ...
            if next_product and (next_product == new_wip):
                ...
                if next_prod_knockout:
                    ...
                    machine_productivity = self.productivity_map[machine][
                        _start:_stop
                    ]
                    ...
                    modified_wip_production[next_product] = (
        ...
        for wip_code, production in modified_wip_production.items():
            wip_cost_contributions[wip_code] = self._soft_solution_costs(
                wip_code, production
            )
    ...
    def _run_solution_loop(self):
        ...
        for x in range(self.iterations):
            ...
            if row < self.solution_max_index:
                next_product = self.solution[machine][
                    row + self.min_swap_hours + 1
                ]
            else:
                next_product = None
            ...
            if new_cost < current_best_cost:
                ...
                acceptance = exp(
                    ((current_best_cost - new_cost) / current_best_cost)
                    * 100
                    / self.temperature
                    + 0.00001
                )
...
class Results:
    def __init__(self, solved_problem):
        ...
        self.machine_name_map = dict(
            zip(self.solver.machine_ids, current_app.config["MACHINE_NAMES"])
        )
    ...
    def _get_shift_rotation(self):
        ...
        soln_df["Shift Start"] = pd.to_datetime(
            soln_df["Shift Start"]
    ...
    def _get_machine_utilisation(self):
        wc_dates = (
            np.array(self.daterange).reshape((-1, 168))[:, :1].ravel().tolist()
        )
        ...
        for machine_id, possible_production in self.productivity_map.items():
            ...
            can_produce = (
                np.where(possible_production > 0, 1, 0)
                .reshape((-1, 168))
                .sum(axis=1)
                .astype(float)
            )
            did_produce = (
                np.where(actual_production > 0, 1, 0)
                .reshape((-1, 168))
                .sum(axis=1)
                .astype(float)
            )

            weekly_utilisation = np.divide(
                did_produce,
                can_produce,
            ...
            utilisation.append(
                {
                    "machine_name": self.machine_name_map[machine_id],
    ...
    def get_solution(self):
            ...
            results.append(
                {
                    ...
                    "demand": self.wip_demands[product_id]
                    .astype(int)
                    .tolist(),
        ...
        rtn["convergence"] = {
            "x": [item[0] for item in self.convergence][::20],
