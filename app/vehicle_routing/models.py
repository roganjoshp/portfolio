...
class RoutingProblem:
    ...
    def build_matrix(self):
        ...
        loc_string = [
            "{},{}".format(
                current_app.config["WH_LON"], current_app.config["WH_LAT"]
            )
        ] + loc_string
        location_map["warehouse"] = (
            current_app.config["WH_LAT"],
            current_app.config["WH_LON"],
        )

        # Call OSRM
        query = (
            current_app.config["OSRM_BASE"]
            + ";".join(loc_string)
            + current_app.config["OSRM_END"]
        )
        ...
        distances = [
            item / 1000
            for sublist in matrix.get("distances")
            for item in sublist
        ]
        durations = [
            item / 60
            for sublist in matrix.get("durations")
            for item in sublist
        ]
        ...
        for i, loc_pair in enumerate(all_location_pairs):
            ...
            final_matrix[(loc_pair[0], loc_pair[1])] = (
                distances[i],
                durations[i],
            )
    ...
    def solve_route(self):
        ...
        algo_config = {"numberOfIterations": "100", "isInfinite": "false"}
    ...
    def process_results(self, result, matrix, location_coords):
        location_dict = {
            i: location for i, location in enumerate(self.locations)
        }
        ...
        for driver_name, route in raw_routes.items():
            ...
            for i, job in enumerate(route):
                ...
                if i == 0:
                    ...
                    waiting_time = int(
                        float(job["num_departure"])
                        - 5
                        - float(job["num_arrival"])
                    )

                    first_name, surname = location_dict[tracking][
                        "name"
                    ].split(" ")
                    ...
                    # Grab some stats
                    travel_distance, travel_time = matrix[
                    ...
                    all_stats[driver_number]["time"] = (
                        all_stats[driver_number].get("time", 0) + travel_time
                    )
            ...
            all_stats[driver_number]["had_lunch"] = all_stats[
                driver_number
            ].get("had_lunch", False)
    ...
    def build_locations(req):
        ...
        names = np.random.choice(
            current_app.config["CUSTOMER_NAMES"], samples, replace=False
        ).tolist()
        jsprit_starts = np.random.choice(
            current_app.config["START_TIMES"], samples, replace=True
        )
        ...
        details = [
            f"{name}\n{starts[i]}-{ends[i]}" for i, name in enumerate(names)
        ]
